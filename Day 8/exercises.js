...
for(const user of Object.keys(users)){
  if(users[user].skills.length > maxSkill){
    ...
    maxSkillUser = Object.assign({}, users[user]);
  }
}
console.log(maxSkillUser.email, maxSkill);
...
const personAccount = {
  ...
  totalIncome: function(){
    total = 0;
    for(income of Object.keys(personAccount.incomes))
      total += personAccount.incomes[income];
      return total; 
  },
  totalExpenses: function(){
    total = 0;
    for(expense of Object.keys(personAccount.expenses))
      total += personAccount.expenses[expense];
      return total;
  },
  accountInfo: function(){
    console.log(personAccount.firstName, personAccount.lastName);
...
const users3 = [
  ...
  function signUp(name, email, password){
    for(const user of users3)
      ...
      for(const char of '_______'){
        let random = parseInt(Math.random() * alphaNumbs.length);
        randomCode.push(alphaNumbs[random]);
      }
      let id = randomCode.join('');
